import java.util.*;

public class PriorityScheduling {

    public static void main(String[] args) {
        int n = 5;       // Number of Processes
        int CPU = 0;     // CPU Current time
        int allTime = 0; // Time needed to finish all processes

        int[] arrivaltime = {0, 5, 12, 2, 9};
        int[] bursttime = {11, 28, 2, 10, 16};
        int[] priority = {2, 0, 3, 1, 4};
        int[] ATt = new int[n];      // Arrival Time Tracker
        int[] PPt = new int[n];      // Priority Tracker
        int[] waitingTime = new int[n];   // Waiting Time for each process
        int[] turnaroundTime = new int[n]; // Turnaround Time for each process

        // Initialize Arrival Time Tracker and Priority Tracker
        for (int i = 0; i < n; i++) {
            PPt[i] = priority[i];
            ATt[i] = arrivaltime[i];
        }

        int LAT = 0; // Last Arrival Time
        for (int i = 0; i < n; i++) {
            if (arrivaltime[i] > LAT) {
                LAT = arrivaltime[i];
            }
        }

        int MAX_P = 0; // Max Priority
        for (int i = 0; i < n; i++) {
            if (PPt[i] > MAX_P) {
                MAX_P = PPt[i];
            }
        }

        int ATi = 0;     // Pointing to Arrival Time index
        int P1 = PPt[0]; // Pointing to 1st priority value
        int P2 = PPt[0]; // Pointing to 2nd priority value

        int NoP = n; // Number of processes
        int j = -1;

        // Simulation of Non-Preemptive Priority Scheduling
        while (NoP > 0 && CPU <= 1000) {
            for (int i = 0; i < n; i++) {
                if (ATt[i] <= CPU && ATt[i] != (LAT + 10)) {
                    if (PPt[i] != (MAX_P + 1)) {
                        P2 = PPt[i];
                        j = 1;

                        if (P2 < P1) {
                            j = 1;
                            ATi = i;
                            P1 = PPt[i];
                            P2 = PPt[i];
                        }
                    }
                }
            }

            if (j == -1) {
                CPU++;
                continue;
            } else {
                waitingTime[ATi] = CPU - ATt[ATi];
                CPU = CPU + bursttime[ATi];
                turnaroundTime[ATi] = CPU - ATt[ATi];
                ATt[ATi] = LAT + 10;
                j = -1;
                PPt[ATi] = MAX_P + 1;
                ATi = 0;        // Reset pointing to Arrival Time index
                P1 = MAX_P + 1; // Reset pointing to 1st priority value
                P2 = MAX_P + 1; // Reset pointing to 2nd priority value
                NoP = NoP - 1;  // One process completed
            }
        }

        // Output process details
        System.out.println("\nProcess_Number\tBurst_Time\tPriority\tArrival_Time\tWaiting_Time\tTurnaround_Time\n");
        for (int i = 0; i < n; i++) {
            System.out.println("P" + (i + 1) + "\t\t" + bursttime[i] + "\t\t" + priority[i] + "\t\t" + arrivaltime[i] + "\t\t" + waitingTime[i] + "\t\t" + turnaroundTime[i]);
        }

        // Calculate and print average waiting time and turnaround time
        float AvgWT = 0;  // Average waiting time
        float AVGTaT = 0; // Average Turnaround time
        for (int i = 0; i < n; i++) {
            AvgWT += waitingTime[i];
            AVGTaT += turnaroundTime[i];
        }

        System.out.println("Average waiting time = " + (AvgWT / n));
        System.out.println("Average turnaround time = " + (AVGTaT / n));
    }
}
